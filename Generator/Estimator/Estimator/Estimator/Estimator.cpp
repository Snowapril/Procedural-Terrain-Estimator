...
int Estimator::descentTabling() {
	...
	// ?òÏ†ï?†Í≤É : local minimaÍ∞Ä ?ÑÎãà??global minimaÎ•?Íµ¨ÌïòÍ≥??àÏúºÎØÄÎ°?descent table????Ï±ÑÏö¥ ?§Ïùå forÎ¨??åÎ©¥??Ï£ºÎ????êÍ∏∞Î≥¥Îã§ ??Í∞íÏù¥ ?ÜÎäî ?†Î? local minimaÎ°?ÏßëÏñ¥?£Ïñ¥?ºÌï®
...
pixel randFill(int areaHeight, int wetDistance, int y,int x) {
	...
	// Height : ?πÏ†ï ?Ä?ºÏù¥ Ï°¥Ïû¨?òÎäî ?íÏù¥???úÏûë
...
void Estimator::blendmapColoring() {
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++) {
			...
			//Í∑ºÏ≤ò??local minima?Ä??Í±∞Î¶¨Ï∞? ?íÏù¥Ï∞®Ïóê ?∞Îùº??Î¨ºÏù¥ Í≥†Ïù¥???ïÎèÑÍ∞Ä ?¨ÎùºÏß?
			int des_y = descentTable[i][j].first, des_x = descentTable[i][j].second;
			int wet_dist = (i - des_y) * (i - des_y) + (j - des_x) * (j - des_x);

			//Í≥†Î†§ ?ÑÎ≥¥ :  local minima?Ä??Í±∞Î¶¨Ï∞? local minima?Ä???íÏù¥Ï∞?
			//wet_distÍ∞Ä Ïª§Ïßà?òÎ°ù ?îÏÑùÏßÄ??
