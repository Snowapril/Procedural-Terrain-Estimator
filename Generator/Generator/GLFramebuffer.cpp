...
void GLFramebuffer::attachColorTexture(int width, int height, uint32_t wrapMethod, bool hdr)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, hdr ? GL_RGBA16F : GL_RGB, width, height, 0, hdr ? GL_RGBA : GL_RGB, hdr ? GL_FLOAT : GL_UNSIGNED_SHORT, nullptr);
...
void GLFramebuffer::bindFramebuffer(int startX, int startY, int width, int height) const
{
	glBindFramebuffer(GL_FRAMEBUFFER, FBO);
...
void GLFramebuffer::unbindFramebuffer(int startX, int startY, int width, int height) const
{
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	glViewport(startX, startY, width, height);
}

void GLFramebuffer::unbindFramebuffer(const Util::Rect & rect) const
{
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	...
	glViewport(leftTop.x, leftTop.y, scale.x, scale.y);
}
